  ...
  try {
    ...
    const sessionConfig = {
      session: {
        ...
        instructions,
        // you can change voice name if your account supports others
        audio: {
          output: { voice: "marin" },
        },
      },
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const url = new URL(`https://api.openai.com/v1/realtime?model=${encodeURIComponent(model)}`);
        ...
        const r = https.request(
          {
            ...
            method: "POST",
            agent: openaiAgent, // you already created this
            headers: {
              Authorization: `Bearer ${apiKey}`,
              "Content-Type": "application/sdp",
              "Content-Length": Buffer.byteLength(offerSdp),
            },
          },
          (resp) => {
            let data = "";
            ...
            resp.on("end", () => {
              if (resp.statusCode >= 200 && resp.statusCode < 300) {
                ...
                reject(new Error(`OpenAI SDP exchange failed (${resp.statusCode}): ${data}`));
              }
            });
          }
        );
